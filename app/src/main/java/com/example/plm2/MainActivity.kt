...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       // Это кнопка для открытия медиатеки
        ...
        // val trackList = createTrackList()          ОШИБКА НЕ ИСПОЛЬЗУЕТСЯ!!!

        // Создаем адаптер
        // val trackAdapter = TrackAdapter(trackList)               ОШИБКА НЕ ИСПОЛЬЗУЕТСЯ!!!

        // Настраиваем RecyclerView
        //recyclerView.layoutManager = LinearLayoutManager(this)            ОШИБКА ТУТ!!!
        //recyclerView.adapter = trackAdapter                     ОШИБКА ТУТ!!!
    }

    private fun createTrackList(): List<Track> {
        // Возвращаем список треков, например, как было ранее в вашем коде
        return arrayListOf(
